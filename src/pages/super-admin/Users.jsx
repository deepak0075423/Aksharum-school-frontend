...
export default function SAUsers() {
  ...
  const handleImport = async (e) => {
    ...
    try {
      ...
      const res = importRole === 'teacher' ? await api.bulkTeachers(fd) : await api.bulkStudents(fd);
      let created = 0; let failed = 0;
      if (typeof res === 'string') {
        // students endpoint streams SSE frames; read counts from the final frame
        for (const chunk of res.split('\n\n')) {
          if (!chunk.startsWith('data: ')) continue;
          try {
            const evt = JSON.parse(chunk.slice(6));
            if (evt.type === 'done') { created = evt.created ?? 0; failed = evt.errors?.length ?? 0; }
      ...
      else if (created > 0) toast.success(`Imported ${created} user(s)`);
      else toast.error('No rows found in the file');
      if (created > 0) { setImportOpen(false); setImportFile(null); setImportSchool(''); }
      refetch();
    } catch (err) { toast.error(err.message); }
